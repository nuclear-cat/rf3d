...
class PlaceRepository extends Repository\ContentRepository
{
    ...
    private function createPlacesQueryBuilder($citySlug, $categorySlug, $districtSlug, $onlyMainPage)
    {
        ...
        if ($categorySlug) {
            $qb->join('p', 'bolt_relations', 'cr', '((cr.from_contenttype = "places" AND cr.from_id = p.id AND cr.to_contenttype = "categories") OR (cr.from_contenttype = "categories" AND cr.to_id = p.id AND cr.to_contenttype = "places"))');
